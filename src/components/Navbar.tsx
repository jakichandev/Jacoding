...
const Navbar = ({ navState = "sm" }: NavbarProps) => {
  ...
  return (
    <nav
      ...
      className={`fixed bg-theme-gray-900/80 z-[100] rounded-lg border-2 border-white/30 px-2 ml-sections-mobile md:ml-sections mt-6 overflow-hidden flex flex-col items-center animate-navbar transition-all duration-500 overflow-y-auto cursor-pointer shadow-xl focus:outline-none focus:ring-2 focus:ring-theme-aqua-500 focus:ring-offset-2 focus:ring-offset-theme-gray-950 ${
    ...
    >
      ...
      <ul 
        className={navActualState === "sm" ? "hidden" : "flex flex-col gap-4 my-3 w-full"}
        role="list"
        aria-label="Pagine del sito"
      >
        {routes.map((route) => (
          <li key={route.id} className="w-full px-4 rounded-lg border-2 border-white/20 hover:border-white/50 focus-within:border-white/50 relative overflow-hidden transition-all duration-300 hover:scale-[1.02]">
            ...
            >
              <span className="w-8 h-8 flex items-center justify-center brightness-110 contrast-125 drop-shadow-[0_0_8px_rgba(78,205,196,0.6)]" aria-hidden="true">
                <img className="w-full h-full object-contain" src={route.icon} alt="" />
              </span>
        ...
        <li className="flex gap-2 justify-center" role="list" aria-label="Link social">
          <Link 
          ...
          >
            <img src={github} alt="" className="icon-btn rounded-lg brightness-110 contrast-125" aria-hidden="true" />
          </Link>
        </li>
      </ul>
    </nav>
