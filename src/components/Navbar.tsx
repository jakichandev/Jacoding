...
const Navbar = ({ navState = "sm" }) => {
  ...
  return (
    <nav
      ...
      className={`relative rounded-lg border-1 px-2 border-opacity ml-3 mt-3 overflow-hidden flex flex-col items-center animate-navbar transition-all duration-500 ${
    ...
    >
      ...
      <ul className={navActualState === "sm" ? "hidden" : "flex flex-col gap-4 my-3 w-full"}>
        {routes.map((route) => (
          <li key={route.id} className="w-full px-4 rounded-lg border-1 border-opacity">
            <Link to={route.path} className="flex items-center gap-3 text-sm  text-txt-2 hover:text-primary my-2">
              <span><img className="w-8" src={route.icon} alt={route.name}  /></span><h4 className="font-lg">{route.name}</h4>
            </Link>
          </li>
        ))}
      </ul>
