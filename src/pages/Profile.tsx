      ...
      <section className="py-30 px-sections-mobile md:px-sections grid grid-cols-1 lg:grid-cols-[1fr_4fr] text-txt font-p-1 min-h-[90vh] gap-x-8 gap-y-3">
        ...
        <GlassContainer opacity="70" variant="default" className="h-fit">
          ...
          <ul className="flex flex-col text-lg text-white list-none font-p-1 font-bold gap-2 mt-4">
            {profile.contacts.map((contact, index) => (
              <li key={index} className="hover:text-theme-aqua-500 transition-colors">
        ...
        <div className="flex flex-col gap-6">
          <GlassContainer opacity="70" variant="default">
            <Heading text="About me" className="text-4xl md:text-5xl text-white mb-4" />
          ...
          <GlassContainer opacity="70" variant="default">
            <Heading text="Esperienze Lavorative" className="text-3xl md:text-4xl text-white mb-4" />
              ...
              {profile.workExperience.map((experience) => (
                ...
                >
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    <span className="text-theme-aqua-500 font-semibold">{experience.when.from}</span>
                    <span className="text-white/60">-</span>
                    <span className="text-theme-aqua-500 font-semibold">{experience.when.to}</span>
                  </div>
                  <Heading text={experience.name} className="text-2xl md:text-3xl text-white mb-2" />
